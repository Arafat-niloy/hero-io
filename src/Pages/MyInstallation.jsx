    ...
    <div className="p-10">
      ...
      <div className="grid grid-cols-1 gap-6">
        {installedApps.map((app) => (
          <div
            key={app.id}
            className="bg-white rounded px-1 md:pr-4 lg:pr-0 gap-1   shadow-xl flex items-center"
          >
            <div className=" lg:w-2/12 ">
              <img
                src={app.image}
                alt={app.title}
                className=" h-15 md:h-30 md:w-30 object-cover md:mx-auto"
              />
            </div>
            <div className=" w-8/12 md:space-y-3">
              <h2 className="md:text-2xl font-bold">{app.title}</h2>

              <div className="flex gap-2 text-sm md:text-lg lg:gap-6">
                <p className="text-[#00D390] text-sm md:text-lg font-semibold flex items-center gap-2">
                  <img src={dwnldImg} alt="" className="w-3 h-3 md:w-5 md:h-5" />
                  {app.downloads}M
                </p>
                <p className="flex items-center md:text-lg font-semibold  gap-2 ">
                  <img src={ratingImg} alt="" className="w-3 h-3 md:w-5 md:h-5" />
                  {app.ratingAvg}
                </p>
                <p className="flex items-center md:text-lg font-semibold text-gray-500  gap-2 ">
                  {app.size} MB
                </p>
              </div>
            </div>
